...
object CommandParser {
    /**
     * Parses a free-form user utterance and maps it to a corresponding ParsedCommand intent.
     *
     * Normalizes and cleans the input, then recognizes common command types such as YouTube play/search, open app,
     * call contact, send SMS, torch control, time/date/battery/volume queries, timers, web searches, camera actions,
     * and settings navigation; returns Unknown when no pattern matches.
     *
     * @param rawText The raw user-provided text or voice transcription to parse.
     * @return A ParsedCommand representing the interpreted intent (for example `OpenApp`, `CallContact`, `SendSms`, `TurnTorch`,
     * `SetTimer`, `WebSearch`, `OpenCamera`, `OpenWifiSettings`, etc.), or `Unknown` if the input cannot be mapped.
     */
